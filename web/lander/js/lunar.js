...
audio.puh.src = '..lander/sound/puh.mp3';
...
function doEvent(kind, ms) {
    ...
    if (kind == 'lose')
    {
        ...
        $('#explode').css('background', 'url(\'./lander/img/explode.gif?p=' + new Date().getTime() + '\')');
    }
...
$(document).ready(function () {
    ...
    window.setInterval(function () {
        if (!pause) {
            ...
            $('#ship').css('background', (spacePressed & ship.fuel > 0) ? 'url(\'./lander/img/ship.png\')' : 'url(\'./lander/img/shipOff.png\')');
            $('#ms').html(getSpeed() + ' m/s');
            document.form.x.value = ship.v;
